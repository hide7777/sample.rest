...
Postmanなどの、REST APIを呼び出せるツール使って、以下APIの動作確認が出来ます。<BR>

| メソッド | 種類 | URL | 説明 |
| :---| :---: |:---|:---|
| ユーザID検索| GET| /api/v1/users/{id}|PKキーである{id}が完全一致するユーザ情報をJSONで返却します。{id}を指定しない場合には、全データを返却します。|
| 曖昧検索| GET| /api/v1/users/search|リクエストパラメータで{name}と{email}を指定してください。片方または両方の値を使ったLIKE文での曖昧検索（部分一致検索）結果を返却します。{name}と{email}の両パラメータは必須ですが、{name}と{email}の両方に値を指定しない場合には、全データを返却します。|
| ユーザ登録・変更| POST| /api/v1/users/|リクエストのBODYに、ユーザデータ(数値型{id},文字列{name},文字列{email},文字列{startdate},数値{lank})をJSON形式で定義して下さい。JSONデータの内容をデータベースに登録します。また、{id}が一致するデータが存在する場合には上書きします。{name}に格納する氏名の文字コード形式は、UTF-8日本語とします。|
| ユーザ削除| DELETE| /api/v1/users/{id}|指定された{id}と一致するユーザデータをデータベースから削除します。|
